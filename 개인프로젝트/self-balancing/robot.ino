...
// use the software serial to connect to the wireless module; use instead
// of the HW serial in order to let the HW serial to be able to print
// debug messages on the terminal on the laptop; after all the debug stuff
// I could switch back to using the HW serial as in the original code;
#include <SoftwareSerial.h>

// connect the TX of wirelss module to Arduino pin 6 RX;
// connect the RX of wirelss module to Arduino pin 7 TX;
SoftwareSerial serial_SW_wireless_module(8, 9); // (RX, TX)
...
float pid_d_gain = 15;                                       //Gain setting for the D-controller (30)
...
void setup(){
  ...
  serial_SW_wireless_module.begin(38400);                                    // start SW serial that connects wireless modeul to Arduino;
  //Serial.begin(9600);                                                       //Start the serial port at 9600 kbps
  

  Serial.begin(19200);                                                       //Start the serial port at 9600 kbps
